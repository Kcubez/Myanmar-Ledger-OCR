...
export default function LedgerWorkspace() {
  ...
  return <div className="shell">
    <header><p className="eyebrow">MYANMAR LEDGER OCR</p><h1>စာရင်းစာရွက်ကို ဖတ်ထုတ်ပါ</h1><p>ပုံ 1 မှ 5 ပုံအထိတင်ပြီး Revenue, Expense, Profit, Loss ကိုပြန်စစ်နိုင်သည်။</p><div className="workflow-hint"><span>1. ပုံရွေးပါ</span><span>2. Gemini ဖြင့်ဖတ်ပါ</span><span>3. စစ်ပြီး သိမ်းပါ</span></div></header>
    <section className="key-panel card"><div><p className="eyebrow">GEMINI KEY STATUS</p><h2>Vercel API keys</h2><p className="muted">Browser keys {browserKeys.length ? `${browserKeys.length} ခု သိမ်းထားသည်` : "မသိမ်းရသေးပါ"}။</p></div><div className="key-actions"><button className="secondary" type="button" onClick={() => browserKeyDialogRef.current?.showModal()}>Manage browser keys</button><button className="secondary" type="button" onClick={() => setShowKeyIds(!showKeyIds)}>{showKeyIds ? "Hide IDs" : "Show IDs"}</button><button type="button" onClick={() => void testEnvironmentKeys()} disabled={testingKeys || !environmentKeys.length}>{testingKeys ? "Testing…" : "Test Vercel keys"}</button></div><div className="key-list">{environmentKeys.length ? environmentKeys.map((key) => <div className="key-row" key={key.slot}><label>Key {key.slot}</label><output>{showKeyIds ? key.masked : "••••••••••••"}</output><span className={`key-state ${key.state}`}>{keyLabel[key.state]}</span></div>) : <p className="muted">No environment keys found.</p>}</div>{keyStatus && <p className="status" role="status">{keyStatus}</p>}</section>
    <dialog className="key-dialog" ref={browserKeyDialogRef}><div className="key-dialog-content"><div className="dialog-heading"><div><p className="eyebrow">BROWSER-ONLY SETTINGS</p><h2>Browser API keys</h2><p className="muted">ဒီ browser/device ထဲတွင်သာ သိမ်းသည်။ Browser key ရှိလျှင် extraction မှာ Vercel keys ထက်အရင်သုံးမည်။</p></div><button className="delete" type="button" aria-label="Close browser key settings" onClick={() => browserKeyDialogRef.current?.close()}>×</button></div>{browserKeys.length ? browserKeys.map((key, index) => <div className="browser-key-row" key={key.id}><input aria-label={`Key ${index + 1} label`} value={key.label} placeholder={`Key ${index + 1}`} onChange={(event) => updateBrowserKey(key.id, "label", event.target.value)} /><input aria-label={`Key ${index + 1} value`} type={visibleBrowserKeys.includes(key.id) ? "text" : "password"} value={key.value} placeholder="Paste Gemini API key" onChange={(event) => updateBrowserKey(key.id, "value", event.target.value)} /><button className="secondary" type="button" onClick={() => setVisibleBrowserKeys((items) => items.includes(key.id) ? items.filter((id) => id !== key.id) : [...items, key.id])}>{visibleBrowserKeys.includes(key.id) ? "Hide" : "Show"}</button><button className="delete" type="button" aria-label={`Remove ${key.label || `Key ${index + 1}`}`} onClick={() => setBrowserKeys((items) => items.filter((item) => item.id !== key.id))}>×</button></div>) : <p className="muted">Browser key မထည့်ရသေးပါ။</p>}<div className="browser-key-actions"><button className="secondary" type="button" onClick={() => setBrowserKeys((items) => items.length >= 10 ? items : [...items, { id: crypto.randomUUID(), label: `Browser Key ${items.length + 1}`, value: "" }])}>+ Add key</button><button type="button" onClick={() => void persistBrowserKeys()}>Save browser keys</button><button type="button" onClick={() => void testBrowserKeys()} disabled={testingKeys || !browserKeys.some((key) => key.value.trim())}>{testingKeys ? "Testing…" : "Test browser keys"}</button></div>{browserKeyResults.length > 0 && <div className="browser-key-results">{browserKeyResults.map((key, index) => <div className="key-row" key={key.slot}><label>{browserKeys.filter((item) => item.value.trim())[index]?.label || `Browser Key ${key.slot}`}</label><output>{showKeyIds ? key.masked : "••••••••••••"}</output><span className={`key-state ${key.state}`}>{keyLabel[key.state]}</span></div>)}</div>}</div></dialog>
    <section className="upload card"><label className="file"><input type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={chooseFiles} />ပုံရွေးပါ</label><span>{files.length ? `${files.length} / 5 ပုံရွေးထားသည်` : "JPG, PNG, WEBP · ပုံ 5 ပုံအထိ · တစ်ပုံ 4 MB"}</span><button onClick={() => void extractBatch()} disabled={!files.length || !!batchProgress}>{batchProgress ? `${batchProgress.current}/${batchProgress.total} ဖတ်ထုတ်နေသည်…` : "Gemini ဖြင့် ဖတ်ထုတ်ပါ"}</button>{files.length > 0 && <div className="file-list">{files.map((file, index) => <span key={`${file.name}-${index}`}>{file.name}<button className="delete" aria-label={`Remove ${file.name}`} disabled={!!batchProgress} onClick={() => setFiles(files.filter((_, itemIndex) => itemIndex !== index))}>×</button></span>)}</div>}{batchProgress && <p className="status" role="status" aria-live="polite">{batchProgress.name} ကို ဖတ်ထုတ်နေသည်…</p>}{status && <p className="status" role="status">{status}</p>}{batchErrors.length > 0 && <ul className="batch-errors" role="alert">{batchErrors.map((error) => <li key={error}>{error}</li>)}</ul>}</section>
